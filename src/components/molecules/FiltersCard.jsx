...
const FiltersCard = ({
  ...
  /* Solo ofertas (existente) */
  onlyOffers,
  setOnlyOffers,

  /* Region / Pais */
  regions,              // string[] - lista de regiones disponibles
  selectedRegions,      // string[] - regiones seleccionadas
  setSelectedRegions,   // (regions: string[]) => void

  /* Duracion en dias */
  durationRange,        // { min: number, max: number } | null
  setDurationRange,     // (range | null) => void

  /* Estrellas del hotel */
  starFilter,           // number[] - estrellas seleccionadas [3, 4, 5]
  ...
  /* Porcentaje de descuento minimo */
  minDiscount,          // number | null
  setMinDiscount,       // (val: number | null) => void

  /* Tipo de pension */
  boardType,            // 'half' | 'full'
  setBoardType,         // (val: 'half' | 'full') => void
      ...
      {setBoardType && (
        <div className="mb-6">
          ...
          <div className="flex rounded-lg overflow-hidden border border-surface-700">
            <button
              type="button"
              onClick={() => setBoardType('half')}
              className={`flex-1 px-3 py-2 text-xs font-medium transition-colors
                ${boardType === 'half'
                  ? 'bg-brand-500 text-white'
                  : 'bg-surface-800 text-ink-soft hover:bg-surface-700'
                }`}
            >
              Media pension
            </button>
          ...
          <div className="flex flex-wrap gap-2">
            {DURATION_OPTIONS.map(option => {
              const isActive = durationRange &&
                durationRange.min === option.min &&
                durationRange.max === option.max
              return (
                <button
                  key={option.label}
                  type="button"
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors
                    ${isActive
                      ? 'bg-brand-500 text-white'
                      : 'bg-surface-700 text-ink-soft hover:bg-surface-600'
                    }`}
                >
                  {option.label}
                </button>
